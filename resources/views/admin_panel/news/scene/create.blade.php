...
@section('content')
    ...
    <div class="col-12">
        <div class="box  box-primary">
            ...
            <div class="box-body">
                <form class="form-horizontal" method="POST" action="{{route('admin.scene.store')}}" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-6">
                            <select autocomplete="off" class="filter form-control" id="news_scene_group_id" name="news_scene_group_id">
                                ...
                                @foreach($scene_groups as $group)
                                    <option value="{{$group->id}}" {{old('news_scene_group_id') ==  $group->id  ? 'selected' : ''}}>{{$group->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
