<table class="table table-hover">
    ...
    @forelse($categories as $row)
        <tr>
            <td nowrap width='3%'>
                <form class='form-inline' action="{{route('admin.shop.researches.category.destroy', [$row->id])}}" method="POST" id='formDelete{{$row['id']}}'>
            ...
            <td>
                <a href="{{route('admin.shop.researches.category.edit', [$row->id])}}" data-toggle="tooltip" data-placement="top" title="Редактировать категорию">
                    {{$row['name']}}
                </a>
            </td>
            {{--<td>
        ...
        @forelse ($row->child as $child)
            <tr>
                <td nowrap width='3%'>
                    <form class='form-inline' action="{{route('admin.shop.researches.category.destroy', [$child->id])}}" method="POST" id='formDelete{{$child->id}}'>
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    </form>
                ...
                <td colspan="2">
                    <a href="{{route('admin.shop.researches.category.edit', [$child->id])}}" data-toggle="tooltip" data-placement="top" title="Редактировать категорию">
                        {{$row->name}} > {{$child->name}}
                    </a>
                </td>
                <td>
