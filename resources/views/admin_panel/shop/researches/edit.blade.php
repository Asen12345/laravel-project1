    ...
    <div class="col-12">
        <div class="box  box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Редактирование Исследования</h3>
            </div>
            <div class="box-body">
                <form class="form-horizontal" method="POST" action="{{route('admin.shop.researches.update', ['id' => $researches->id ])}}" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <input id="title" name="title" type="text" class="form-control" value="{{ $researches->title }}" required="">
                        </div>
                    </div>
                            ...
                            <select class="filter form-control js-example-basic-multiple" id="shop_category_id" name="shop_category_id[]" multiple="multiple" required autocomplete="off">
                                @forelse($categories as $category)
                                    @if($category->parent_id == 0)
                                        <option value="{{ $category->id }}" {{ $researches->category->contains($category->id) ? 'selected' : ''}}>{{ $category->name}}</option>

                                        @forelse($categories as $subcategory)
                                            @if($subcategory->parent_id == $category->id)
                                                <option value="{{ $subcategory->id }}" {{ $researches->category->contains($subcategory->id) ? 'selected' : ''}}>{{ '-- ' . $subcategory->name}}</option>
                                            @endif
                                        @empty
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <textarea id="annotation" name="annotation" class="form-control summernote" rows="4" cols="50" required="">{{ $researches->annotation }}</textarea>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <textarea id="content" name="content" class="form-control summernote" rows="4" cols="50">{{ $researches->content }}</textarea>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <textarea id="table" name="table" class="form-control summernote" rows="4" cols="50">{{ $researches->table }}</textarea>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <img id="user_avatar" class="img-fluid" src="{{ $researches->image ?? url('/img/no_picture.jpg')}}" width="200px" alt="Image">
                            <input type="hidden" name="image" id="picture" value="{{ $researches->image }}">
							<p><br /><input type="button" class="btn btn-default" value="Сменить" onclick="select_avatar()"></p>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-7">
                            <input type="file" class="form-control-file" name="demo_researches" id="demo" style="display:none" onchange="handleFiles(this.files, 'demo')">
                                <label for="demo" class="btn btn-primary center-block">{{ $researches->demo_file ?? 'Выберите файл' }}</label>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-7">
                            <input type="file" class="form-control-file" name="file_researches" id="file" style="display:none" onchange="handleFiles(this.files, 'file')">
                            <label for="file" class="btn btn-primary center-block">{{ $researches->file ?? 'Выберите файл' }}</label>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-2">
                            <input id="published_at" name="published_at" type="text" class="form-control datep" value="{{ isset($researches->published_at) ? \Carbon\Carbon::parse($researches->published_at)->format('d-m-Y') : ''}}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="page" class="col-sm-3 col-form-label text-right">Количество страниц</label>
                        <div class="col-sm-6">
                            <input id="page" name="page" type="number" class="form-control" value="{{ $researches->page }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="format" class="col-sm-3 col-form-label text-right">Формат</label>
                        <div class="col-sm-6">
                            <input id="format" name="format" type="text" class="form-control" value="{{ $researches->format }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="language" class="col-sm-3 col-form-label text-right">Язык</label>
                        <div class="col-sm-6">
                            <input id="language" name="language" type="text" class="form-control" value="{{ $researches->language }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="price" class="col-sm-3 col-form-label text-right">Цена, руб.</label>
                        <div class="col-sm-6">
                            <input id="price" name="price" type="number" class="form-control" value="{{ $researches->price }}" required="">
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <select autocomplete="off" id="researches_author_id" name="researches_author_id" type="text" class="form-control" required="">
                                @forelse($authors as $author)
                                    <option value="{{ $author->id }}" {{ $researches->researches_author_id == $author->id ? 'selected' : ''}}>{{ $author->title }}</option>
                                @empty
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <input id="meta_title" name="meta_title" type="text" class="form-control" value="{{ $researches->meta_title }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="meta_keywords" class="col-sm-3 col-form-label text-right">Meta Keywords</label>
                        <div class="col-sm-6">
                            <input id="meta_keywords" name="meta_keywords" type="text" class="form-control" value="{{ $researches->meta_keywords }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="meta_description" class="col-sm-3 col-form-label text-right">Meta Description</label>
                        <div class="col-sm-6">
                            <input id="meta_description" name="meta_description" type="text" class="form-control" value="{{ $researches->meta_description }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="main" class="col-sm-3 form-check-label col-sm-3 text-right">На главной</label>
                        <div class="col-sm-6 form-check">
                            <input id="main" name="main" type="checkbox" class="form-form-check-input" {{ $researches->main == true ? 'checked' : '' }}>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="download" class="col-sm-3 form-check-label col-sm-3 text-right">Количество скачиваний</label>
                        <div class="col-sm-3 form-check">
                            <input id="download" disabled name="download" type="number" class="form-form-check-input" value="{{ $researches->download }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="main" class="col-sm-3 form-check-label col-sm-3 text-right">Количество просмотров</label>
                        <div class="col-sm-3 form-check">
                            <input id="main" disabled name="main" type="number" class="form-form-check-input" value="{{ $researches->views_count }}">
                        </div>
                    </div>
