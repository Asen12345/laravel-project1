...
@section('content')
    @include('admin_panel.topic.answer.partials.answer_filter', ['filter_data' => $filter_data, 'page' => $content['page']])
    <div class="col-12">
        <div class="box  box-primary">
            <div class="box-body table-responsive no-padding">
                @include('admin_panel.topic.answer.partials.answer_table', ['table' => $content['fields'], 'topics' => $answers, 'sort_by' => $sort_by, 'sort_order' => $sort_order])
            </div>
        </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function edit(id, active = null) {
                $.ajax({
                    type: "POST",
                    url: '{{route('admin.answer.main.update')}}',
                    data: {
                        _token:"{{ csrf_token() }}",
                        id: id,
                        active: active
