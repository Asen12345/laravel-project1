<div class="company-activity">
    ...
    <div class="li-tabs tabs-wrapper">
        <ul class="tabs__list mnu-standart-style-reset">
            ...
            <li class="tabs__item company-activity__item">
                <a href="#" data-tab="4" class="tabs__link">
                    <span>Ответов в темах</span>
                    <span class="activity-count">{{$topic_answers_count ?? '0'}}</span>
                </a>
            </li>
        </ul>
        <ul class="tabs__content mnu-standart-style-reset">
            <li data-tab="1" class="tabs__item">
                @foreach ($blog_posts as $post)
                    <div class="company-block">
                        <h2><a href="{{route('front.page.post', ['permission' => 'expert', 'blog_id' => $post->blog->id, 'post_id' => $post->id])}}" class="li-link li-link-blue">{{$post->title}}</a></h2>
            ...
            <li data-tab="4" class="tabs__item hidden">
                @foreach($topicAnswers as $answers)
                    <div class="company-block">
                        <p class="company-block__text">
                            <a href="{{ route('front.page.topic.page', ['url_en' => $answers->topic->url_en]) }}">{!! Str::limit($answers->text,230, ' ...') !!}</a>
                        </p>
                        <div class="blog-row__date">{{\Carbon\Carbon::parse($answers->created_at)->isoFormat("DD MMMM YYYY, H:mm")}}</div>
                    </div>
                @endforeach
                    <div class="col-12">
                        {{$topicAnswers->links('vendor.pagination.custom-front')}}
                    </div>
            </li>
        </ul>
    </div>
